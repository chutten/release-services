...
class Coverity(object):
    ...
    def get_files_with_commands(self):
        ...
        compile_db = json.loads(open(self.compile_commands_path, 'r').read())

        commands_list = []

        for file in self.revision.files:
            # It must be a C/C++ file
            _, ext = os.path.splitext(file)

            if ext.lower() not in settings.cpp_extensions:
                continue
            file_with_abspath = os.path.join(settings.repo_dir, file)
            for f in compile_db:
    ...
    def run(self, revision):
        ...
        assert os.path.exists(
            self.compile_commands_path), 'Missing Coverity in {}'.format(
        ...
        try:
            subprocess.run(cmd, cwd=self.cov_path, check=True, stdout=subprocess.PIPE)
        ...
        for element in commands_list:
            ...
            try:
                subprocess.run(cmd, cwd=element['directory'], check=True, stdout=subprocess.PIPE)
            except subprocess.CalledProcessError as e:
        ...
        for element in commands_list:
            cmd.append(element['file'])
        logger.info('Running Coverity Analysis', cmd=cmd)
        try:
            subprocess.run(cmd, cwd=self.cov_state_path, check=True, stdout=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
